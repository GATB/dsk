...
public:
    ...
    DSK2ASCII () : Tool ("dsk2ascii")
    {
		...
		getParser()->push_front (new OptionNoParam ("-fastq",   "output in fastq format, one kmer per read", false));
		getParser()->push_front (new OptionNoParam ("-fasta",   "output in fasta format, one kmer per read", false));
		getParser()->push_front (new OptionNoParam ("-c",   "output to stdout", false));
    ...
    template<size_t span> struct Functor  {  void operator ()  (Parameter parameter)
    {
		...
		FILE * output = fopen (tool.getInput()->getStr(STR_URI_OUTPUT).c_str(), "wb");
		
		bool fastq_output = tool.getParser()->saw("-fastq");
		
		bool fasta_output = tool.getParser()->saw("-fasta");
		
		if(tool.getParser()->saw("-c")) output = stdout;
		...
		char * qualdummy;
		qualdummy = (char *) malloc((kmerSize+1)* sizeof(char));
		memset(qualdummy,'-',kmerSize);
		qualdummy [kmerSize] = '\0';
        ...
        for (itKmers->first(); !itKmers->isDone(); itKmers->next())
        {
			...
			{
				fprintf(output,">\n");
				fprintf(output,"%s\n",model.toString(count.value).c_str());
			...
			{
				fprintf(output,"%s %i\n",model.toString(count.value).c_str(),count.abundance);
